...
## [2026-06-29] maintenance | Full-vault organization audit
- Added [[vault-organization-audit-2026-06-29]] after a full inventory of routing, Git boundaries, experiment storage, baseline mirrors, temporary files, symlinks, and generated caches.
- Replaced 10 ambiguous Wiki-page links to `index` with explicit relative Markdown links, extended `scripts/lint_wiki.py` to report ambiguous Wikilinks, and refreshed the master index and maintenance router.
- Repaired baseline ignore rules that hid LLAMA-FACTORY data-loader source/tests and reviewed privacy-filter synthetic fixtures; kept unresolved CADET Git LFS sample pointers ignored and aligned `baselines/README.md` with the retained compact artifacts.
- Routed the RA-HMD text architecture exports, added a checksum manifest for the current `newRAHMD` staging bundle, and removed its stale transfer archive.
- Removed seven non-protected `.DS_Store` files and two reproducible `*.egg-info` directories while preserving `raw/`, server mirrors, experiment archives, private gold, same-day `tmp/` JSON files, `.git/`, `.venv/`, and the user's `.obsidian` changes.
- Re-ran `scripts/lint_wiki.py`, `scripts/wiki_inventory.py`, `scripts/check_source_tag_drift.py`, `scripts/check_pdf_text_artifacts.py`, and `git diff --check`; no structural, tag-drift, PDF-artifact, or whitespace failures were reported.

## [2026-06-30] query-answer | LLM-guided hate factor structure induction
- Added [[llm-guided-hate-factor-structure-induction-2026-06-30]] after checking CADET's four-factor causal formulation, PLEAD intent-slot structure, Latent Hatred taxonomy, conversational context, DuPL latent components, community-agent consultation, and HateXScore explanation diagnostics.
- Defined a compact Factor Structure 1.0 around observable communicative function, target structure, target-linked harmful proposition, expression strategy, observable or missing context, evidence, and epistemic state.
- Specified a falsifiable coverage test: label-blind constrained extraction, evidence spans, abstention, a fixed factor-to-verdict rule, stability checks, and explicit separation of schema gaps from missing context, extractor errors, policy disagreement, and label noise.
- Proposed a controlled mapper-auditor-gap-clusterer-proposer-critic-validator loop in which ontology changes are accepted only when they improve held-out structural coverage or stability without increasing leakage or unrestricted catch-all use.
- Routed the new proposal from [[index]], [[hate-speech-research-map]], and [[target-relation-grounding-literature-map]].
- Re-ran `scripts/lint_wiki.py`, `scripts/wiki_inventory.py`, `scripts/check_source_tag_drift.py`, and `scripts/check_pdf_text_artifacts.py`; the wiki now contains 358 pages and 108 concept pages with no reported structural, tag-drift, or PDF-artifact failures.
- Added an operational convergence rule: representational, decision, interventional, and transfer sufficiency; a factor-only versus factor-plus-text residual test; complexity-penalized held-out acceptance; and stopping after three consecutive revision rounds with no accepted ontology change before one untouched test evaluation.
- Froze an explicit Ontology 1.0 discussion draft with five factors, closed enumerations, an auditable JSON record, a deterministic `toxic`/`non_toxic`/`unresolved` derivation rule, and a mapping that separates the current mixed-level IHC `hate_class` values into harm, expression, and narrative-frame fields.
- Added a factor-validity evaluation card covering manual factor agreement, structural coverage and forced fit, concept-style task completeness, factor-plus-text residual gain, evidence sufficiency/comprehensiveness, paraphrase stability, counterfactual isolation, leave-one-factor-out utility, transfer validity, and complexity control.
- Narrowed the actual feasibility Ontology 1.0 to four factors—target relation, coarse harm type, expression style, and observable author stance/context—plus evidence, coverage, and gap notes. Defined a one-pass 200-row diagnostic with no agentic ontology revision; the previously detailed five-factor schema and full validation suite are now explicitly later-iteration candidates.
- Inspected `xu-l20:/data/chenjt/hate/DATA/llm_restructed` and `/data/chenjt/hate/DATA/IHC` read-only. Updated feasibility 1.0 to use list-valued per-target relation/harm records, add a raw Stage-1 explicit-hate style slice, keep `IHC_target_v1/train.json` input text-only, and reserve native/generated fields for post-hoc audit according to their provenance and label exposure.
- Removed self-reported `coverage` from the feasibility mapper output. The mapper now emits uncertainty and schema-gap candidates, while an external deterministic validator assigns `covered`, `partial`, or `unexplained` to prevent self-certifying explanations.
- Added a factor provenance and cost matrix, a staged factor-usage plan, and explicit inference flows. The feasibility run uses one structured call per sampled row; full-corpus annotation is deferred, missing external context is never generated, extracted factors are diagnostic outputs first, and later modeling predicts factors from text instead of accepting gold or completed fields as inference inputs.
- Rechecked the primary CADET and CATCH papers plus the local CADET implementation after the user rejected an overbuilt custom ontology. Replaced the active Version 1.0 with a paper-faithful CADET factor probe: binary `M_probe`, free-text `T_probe`, binary `S_probe`, and fixed `U_metadata`; retained CATCH's two-factor causal/target probe as the minimal baseline and deferred all custom harm, relation, stance, context, evidence, and agent fields.
- Corrected the active Version 1.0 after the user confirmed that their working IHC removes explicit-hate samples. Promoted the CATCH-style `C_probe + T_probe` decomposition to the active experiment, removed style/environment probes, prohibited reintroducing raw explicit-hate rows to rescue style variation, and added target-present non-hate false-positive and target-leakage diagnostics.
- Added a concrete local-first execution plan for the 200-row CATCH probe: frozen split-only sampling, physically separated prompt inputs and hidden gold, five-row dry run, cached OpenAI-compatible runner, strict validation, target normalization, shortcut-aware metrics, a 40-row manual audit, and explicit proceed/stop criteria.
- Tightened the execution boundary by moving the diagnostic `slice` field out of Mapper-visible `inputs.jsonl` and into hidden `gold.local.jsonl`; the inference runner now sees only sample ID and text.
- Replaced the random target-present slice with 50 same-target opposite-label pairs, retained the 100/100 label balance, added hidden `pair_id` metadata and pair-level accuracy, and changed the 40-row manual audit to preserve complete pairs. This controls target identity without adding a new factor or exposing sampling metadata to the LLM. Also made the frozen prompt's hate definition explicit and aligned it with IHC's coded-or-indirect protected-group scope rather than relying on the serving model's unspecified moderation policy.
- Expanded the active CATCH probe with a two-message prompt-injection boundary, line-level prompt rationale, a no-demonstration/no-rationale output boundary, explicit metric formulas, strict handling of invalid outputs, pair-joint scoring, pair-error types, and pair-aware bootstrap intervals. Recorded why Macro F1 alone is invalid evidence of disentanglement: the target-presence shortcut reaches about 0.733 Macro F1 on the diagnostic composition while scoring zero on same-target pair accuracy.
- Corrected the CATCH target semantics after identifying that `non_hate -> T=[]` discarded the completed not-toxic target field and was not supported by CATCH's separate target-label prediction. Version 1.0 now extracts social targets independently of the hate verdict, uses completed not-toxic targets as provenance-separated weak references, reserves `[]` for genuine no-target rows, and evaluates whether matched pairs recover the same T while correctly changing C.
- Implemented the local-first runnable `ihc-catch-probe-v1` bundle with a frozen independent C/T prompt, deterministic 200-row same-target sampler, physically separated text inputs and hidden gold, resumable OpenAI-compatible runner, six-count plus slice/pair evaluator, and 20-pair manual-audit builder. Added the required server-sync manifest; no API call, remote sync, or remote execution was performed.
- During local sample validation, found that `IHC_target_v1.target` includes people, organizations, locations, and other directed-at entities in addition to social groups. Broadened only the observable T prompt/contract to match the real field while retaining C as social-group hate detection; this prevents a dataset-schema mismatch from being counted as factor-extraction failure.
- Replaced the C/T runner's unnecessary `openai` SDK dependency with the same direct `httpx` OpenAI-compatible request pattern used by the active DeepSeek workflow. Verified the existing Vault `.venv` (`httpx 0.28.1`), JSON-mode 400 fallback, strict C/T parsing, and five-row validate-only path without a real API call.

## [2026-07-01] maintenance | Full-vault organization refresh and ReasonRAG ingest
- Refreshed [[vault-organization-audit-2026-06-29]] against the current dirty worktree without reverting or overwriting existing experiment, wiki, Obsidian, baseline, or raw-source changes.
- Detected `raw/sources/20812_Process_vs_Outcome_Rewar.pdf` as a new, non-duplicate source and added [[196-zhang-2025-process-vs-outcome-reward-which-is-better-for-agentic-rag-reinforcement-learning]] after visually checking its dataset and result tables.
- Updated [[sources-index]], [[nlp-research-collection]], [[llm-reasoning-source-hub]], [[llm-reasoning]], [[retrieval-augmented-generation]], and [[index]] to reflect 197 raw PDFs, 196 unique indexed PDF sources, and 53 LLM-reasoning sources.
- Removed only regenerated Finder metadata outside protected `raw/` and remote-mirror boundaries; preserved recent `tmp/` diagnostics, `.venv`, `.git`, server mirrors, experiment archives, and the user's `.obsidian` state.

## [2026-07-11] maintenance | Wiki optimization and publish audit
- Corrected [[196-zhang-2025-process-vs-outcome-reward-which-is-better-for-agentic-rag-reinforcement-learning]] after visually checking Tables 1-3 and Appendix Table 5: preserved the verified process-reward results, marked the paper's backbone-uniformity statement as internally inconsistent, and separated local transfer inferences from direct source claims.
- Updated [[llm-guided-hate-factor-structure-induction-2026-06-30]] so the completed CATCH C/T run is historical Baseline 0 and the active pre-implementation direction is a label-blind CADET-seeded M/T/S factor pool with explicit Mapper, Auditor, binding, held-out revision, and no-final-label boundaries.
- Reconciled durable experiment documentation: recorded Baseline 0 and later target-relation/FineTune aggregate metrics without publishing sample-level outputs, corrected the newRAHMD configuration hash, and marked the 2026-07-06 target-relation staging snapshot as superseded by later huashan changes.
- Tightened baseline publication allowlists: LLAMA-FACTORY source/tests and explicitly reviewed privacy-filter synthetic fixtures are trackable, while unresolved CADET LFS sample pointers, datasets, weights, predictions, traces, and caches remain local.
- Refreshed [[vault-organization-audit-2026-06-29]], the maintenance router, source hub dates, and the master index. The ignored 4.7 GB FineTune relay was preserved as a documented cleanup candidate rather than deleted without a separate retention decision.
- Re-ran `scripts/lint_wiki.py`, `scripts/wiki_inventory.py`, `scripts/check_source_tag_drift.py`, `scripts/check_pdf_text_artifacts.py`, `scripts/regenerate_source_hubs.py`, `git diff --check`, and relevant Python syntax checks; no structural, tag-drift, PDF-artifact, source-hub, whitespace, or syntax failures were reported.
