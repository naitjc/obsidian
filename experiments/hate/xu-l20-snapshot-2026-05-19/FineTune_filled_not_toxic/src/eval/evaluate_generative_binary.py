...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--schema-mode",
        choices=["class_only", "class_target", "full", "target_input_class_only"],
        default="full",
    )
...
def _format_target(target: List[str]) -> str:
    normalized = _normalize_hate_class_list(target)
    return ", ".join(normalized) if normalized else "none"


def build_prompt(text: str, schema_mode: str, allowed_hate_class: List[str], target: List[str] = None) -> str:
    ...
    if schema_mode == "target_input_class_only":
        return (
            "You are a hate-speech analysis assistant. "
            "Given the text and its target, output ONLY valid JSON with key class.\n"
            "- class must be exactly one of: toxic, non_toxic\n"
            "Use the target as context for deciding whether the text attacks that target.\n"
            "Do not output any explanation.\n"
            f"Text: {text}\n"
            f"Target: {_format_target(target or [])}\n"
            "JSON:"
        )
...
def main() -> None:
    ...
    predicts_target = args.schema_mode in {"class_target", "full"}
    target_jaccard_values: List[float] = []
    target_jaccard_by_source = defaultdict(list)

    for row in dataset:
        prompt = build_prompt(row["text"], args.schema_mode, allowed_hate_class, row.get("target", []))
        ...
        if predicts_target and int(row["label_id"]) == 1:
