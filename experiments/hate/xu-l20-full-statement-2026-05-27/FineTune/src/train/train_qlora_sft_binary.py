...
def build_target_json(
    ...
    if schema_mode == "full_statement":
        payload["statement"] = statement if label == "toxic" else []
    return json.dumps(payload, ensure_ascii=False)
...
def _encode_example(
    ...
    answer = build_target_json(label.strip(), hate_class, target, statement, schema_mode, allowed_hate_class)
...
def tokenize_datasets(train_file: str, validation_file: str, tokenizer, max_length: int, schema_mode: str):
    ...
    columns_to_remove = [
        col
        for col in ["id", "text", "label", "label_id", "hate_class", "target", "statement", "source", "split"]
