...
def clean_statement_list(values) -> List[str]:
    ...
    # Some IHC source rows contain malformed serialized lists. Preserve the
    # original statement rather than inventing boundaries between clauses.
    return clean_string_list([text])
    ...
    for idx, row in enumerate(raw_rows):
        ...
        rows.append(
            Example(
                ...
                statement=clean_statement_list(row.get("statement", [])),
                source=source,
                split=split,
            )
        )
