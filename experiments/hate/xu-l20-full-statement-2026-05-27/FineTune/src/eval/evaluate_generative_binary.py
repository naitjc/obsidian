...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--schema-mode",
        choices=["class_only", "class_target", "full", "full_statement"],
        default="full",
    )
...
def build_prompt(text: str, schema_mode: str, allowed_hate_class: List[str]) -> str:
    ...
    output_keys = "class, hate_class, target"
    statement_rule = ""
    if schema_mode == "full_statement":
        output_keys += ", statement"
        statement_rule = "- statement must be a JSON array of strings\n"

    return (
        "You are a hate-speech analysis assistant. "
        f"Given the text, output ONLY valid JSON with keys {output_keys}.\n"
        "- class must be exactly one of: toxic, non_toxic\n"
        "- hate_class must be a JSON array of strings\n"
        f"{allowed_line}"
        "- target must be a JSON array of strings\n"
        f"{statement_rule}"
        f"- If class is non_toxic, use empty arrays for hate_class, target"
        f"{', and statement' if schema_mode == 'full_statement' else ''}\n"
        "Do not output any explanation.\n"
        f"Text: {text}\n"
        "JSON:"
    )
...
def parse_generated_json(text: str, allowed_hate_class: List[str]) -> Dict:
    ...
    output["statement"] = _normalize_string_list(payload.get("statement", []))
...
def jaccard_for_values(gold_values: List[str], pred_values: List[str]) -> float:
    gold_set = {item for item in gold_values}
    pred_set = {item for item in pred_values}
...
def main() -> None:
    ...
    statement_jaccard_values: List[float] = []
    statement_jaccard_by_source = defaultdict(list)
    ...
    for row in dataset:
        ...
        row_statement_jaccard = None
        if int(row["label_id"]) == 1:
            row_target_jaccard = jaccard_for_values(row.get("target", []), parsed["target"])
            target_jaccard_values.append(row_target_jaccard)
            target_jaccard_by_source[row["source"]].append(row_target_jaccard)
            if args.schema_mode == "full_statement":
                row_statement_jaccard = jaccard_for_values(row.get("statement", []), parsed["statement"])
                statement_jaccard_values.append(row_statement_jaccard)
                statement_jaccard_by_source[row["source"]].append(row_statement_jaccard)
        ...
        prediction_rows.append(
            {
                ...
                "gold_statement": row.get("statement", []),
                "pred_id": int(pred_id),
    ...
    statement_jaccard_mean = (
        float(sum(statement_jaccard_values) / len(statement_jaccard_values))
        if statement_jaccard_values
        else 0.0
    )
    statement_jaccard_by_source_mean = {
        source: float(sum(values) / len(values)) if values else 0.0
        for source, values in statement_jaccard_by_source.items()
    }
        ...
        json.dump(
            {
                ...
                "statement_jaccard_toxic_only": statement_jaccard_mean,
                ...
                "statement_jaccard_toxic_only_by_source": statement_jaccard_by_source_mean,
            },
            f,
            indent=2,
            ensure_ascii=False,
