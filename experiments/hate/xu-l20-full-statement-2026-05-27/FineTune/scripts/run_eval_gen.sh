...
if [[ "${DATASET}" != "ihc" && "${DATASET}" != "sbic" ]]; then
  echo "Usage: bash scripts/run_eval_gen.sh [ihc|sbic] [base_model_path] [class_only|class_target|full|full_statement]"
  exit 1
fi

if [[ "${SCHEMA_MODE}" != "class_only" && "${SCHEMA_MODE}" != "class_target" && "${SCHEMA_MODE}" != "full" && "${SCHEMA_MODE}" != "full_statement" ]]; then
  echo "Usage: bash scripts/run_eval_gen.sh [ihc|sbic] [base_model_path] [class_only|class_target|full|full_statement]"
  exit 1
fi
...
MAX_NEW_TOKENS=96
if [[ "${SCHEMA_MODE}" == "full_statement" ]]; then
  MAX_NEW_TOKENS=192
fi
